//DESCRIPTION
//	ManagedVector is an array that manages reference counting
...
namespace gge { namespace utils {
	...
	template <class T_>
	class ManagedVector : RefCounter<ManagedVector<T_> > {
		template<class O_>
		friend class ManagedVector;
		friend class RefCounter<ManagedVector>;
	public:
		ManagedVector() : data(new T_*(nullptr)), size_(new int(0)) 
		{ }

		ManagedVector(int size) : data(new T_*(nullptr)), size_(new int(0)) 
		{
			Resize(size);
		}

		ManagedVector(ManagedVector &buf) : data(buf.data), RefCounter<ManagedVector<T_> >(buf), size_(buf.size_) {
			this->addref();
		}

		ManagedVector &operator =(const ManagedVector &buf) {
		...
		void Resize(int size) {
			...
			if(size==0) {
				...
				T_ *temp=new T_[size];
				if(size>*(this->size_)) {
					std::move(*data, *data+*(this->size_), temp);
				}
				else {
