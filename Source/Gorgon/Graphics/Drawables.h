...
namespace Gorgon { namespace Graphics {
	...
	protected:
		...
		virtual void draw(TextureTarget &target, const Geometry::Point &p, RGBAf color) const = 0;
	};
	...
	protected:
		...
		virtual void drawin(TextureTarget &target, const Geometry::Rectangle &r, RGBAf color) const = 0;
		...
		virtual void drawin(TextureTarget &target, const SizeController &controller, const Geometry::Rectangle &r, RGBAf color) const = 0;
	...
	protected:
		...
		virtual void draw(TextureTarget &target, const Geometry::Point &p, RGBAf color) const override {
			auto size=getsize();
			draw(target, p, {float(p.X+size.Width), float(p.Y)}, {float(p.X+size.Width), float(p.Y+size.Height)}, {float(p.X), float(p.Y+size.Height)}, color);
		}
		...
		/// if statements
		virtual void drawstretched(TextureTarget &target, const Geometry::Rectangle &r, RGBAf color) const {
		...
		/// It might be logical to override this as it is possible to avoid additional function calls and
		/// if statements
		virtual void drawin(TextureTarget &target, const Geometry::Rectangle &r, RGBAf color) const override { 
	...
	protected:
		...
		virtual void drawstretched(TextureTarget &target, const Geometry::Rectangle &r, RGBAf color) const override {
		...
		virtual void drawin(TextureTarget &target, const SizeController &controller, const Geometry::Rectangle &r, RGBAf color) const override {
			target.Draw(*this, controller.GetTiling(), controller.CalculateArea(getsize(), r.GetSize())+r.TopLeft(), color);
		}
	};
